var personListPath = contextPath + "/personSyncLog";
var personTable;
/**
 * 初始化主索引基本身份信息列表
 */
...
function iniList(){
	var personList=$("#personSyncLogList");
		    ...
		    "fnServerData": function(sSource, aoData, fnCallback) {
		    	aoData.push(
	               ...
	               { "name": "filter_frontId_EQ", "value": $("#frontId").val() },
	                ...
	                { "name": "filter_idCard_EQ", "value": $("#idCard").val() },
 ...
 *获取多个复选框的基本身份信息日志id
 */
function getpersonSyncLogListId(){
	var checkedIds = getCheckedIds();
	if(!checkedIds || checkedIds.length==0||checkedIds=="on"){
		layer.alert("至少选择一条基本身份信息日志");
		return false;
	}
	return checkedIds;
}
 ...
 * 同步基本身份信息日志
 */
function syncLogPersonById(){
	var ids=getpersonSyncLogListId();
	if(ids!=false&&ids!="on"){
	layer.confirm("确认同步基本身份信息吗?",function(){
			$.ajax({
				"type" : 'post',
				"url" : contextPath + "/personSyncLog/syncLogPerson",
				"dataType" : "json",
				"data":{"syncLogPersonId":JSON.stringify(ids)},
				"async" : false,
