var path = contextPath + "/certificateSyncLog";
/**
 * 初始化联系人列表
 */
var tableList;
				...
				.dataTable({
					...
					"aoColumns" : [
							...
							}, 
							{
								...
								'sName' : "type",
								 "render": function (data,type,full) {
									   return fn_getDictByKeyCode("indexCertificateTypeCode",data) ;
								  } 
							}, 
							{
								"mData" : 'femCode',
...
function changeSyncStatus(){ 
	...
	if(!ids||ids.length<1){
		layer.alert("至少选择一条证件信息日志");
		return false;
	}
	layer.confirm("确认同步证件信息?",function(){
			$.ajax({
				"type" : 'post',
...
function queryCertificateById(id){
	...
	popup('证件详情',certificatePath+'/queryCertificateById?id='+id,'650','400');
}
