var path = contextPath + "/contactWaySyncLog";
var table;
/**
 * 初始化联系人列表
 */
					...
					"aoColumns" : [
							{
								...
								'mRender' : function(data, type, full) {
									...
									if(full.femState=="0"&&full.syncStatus=="0"){
										content='<input type="checkbox"  id="checkbox_id_'+data+'" name="goodCheckbox"  class="checkbox_id" value="'+data+'">'
									}
									return content;				
								}
							},
							...
							}, 
							{
								...
								'sName' : "type",
								 "render": function (data,type,full) {
									   return fn_getDictByKeyCode("contactWayTypeCode",data) ;
								  } 
							}, 
							{
								"mData" : 'femCode',
...
function changeSyncStatus(){ 
	...
	if(!ids||ids.length<1){
		layer.alert("至少选择一条联系方式日志");
		return false;
	}
	layer.confirm("确认同步联系方式信息?",function(){
			$.ajax({
				"type" : 'post',
