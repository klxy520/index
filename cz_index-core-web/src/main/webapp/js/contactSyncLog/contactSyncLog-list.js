var personListPath = contextPath + "/contactSyncLog";
var personTable;
/**
 * 初始化主索引基本身份信息列表
 */
...
function iniList(){
	...
	if(personList && personList.length>0){
		personTable = personList.dataTable({
		    ...
		    "aoColumns": [
		     {
		        "mData": 'contactSyncLogId',
		        'sClass': 'center',
		        'sName':"contactSyncLogId",
			   ...
			   },
		     {
			     ...
			     'sName':"contactName",
			    "render": function (data,type,full) {
					       return '<a class="particulars" onclick="queryContactDetails('+full.contactId+')" href="javascript:;">'+data+'</a>';
			    } 
			   },
			   {
		    ...
		    "fnServerData": function(sSource, aoData, fnCallback) {
		    	aoData.push(
	               ...
	               { "name": "filter_frontId_EQ", "value": $("#frontId").val() },
	                ...
	                { "name": "filter_contactName_EQ", "value": $("#contactName").val() },
...
function queryContactDetails(id){
	popup('联系人信息详情',contextPath+'/contact/queryContactDetails?id='+id,'650','330');
}
/**
 * 查询单条详细信息
 */
 ...
 *获取多个复选框的联系人信息日志的id
 */
function getContactSyncLogListId(){
	var checkedIds = getCheckedIds();
	if(!checkedIds || checkedIds.length==0||checkedIds=="on"){
		layer.alert("至少选择一条联系人信息日志");
		return false;
	}
	return checkedIds;
}
 ...
 * 同步联系人信息日志
 */
function syncLogContact(){
	var ids=getContactSyncLogListId();
	if(ids!=false&&ids!="on"){
	layer.confirm("确认同步联系人信息吗?",function(){
			$.ajax({
				"type" : 'post',
				"url" : contextPath + "/contactSyncLog/syncLogContact",
				"dataType" : "json",
				"data":{"contactSyncLogId":JSON.stringify(ids)},
				"async" : false,
