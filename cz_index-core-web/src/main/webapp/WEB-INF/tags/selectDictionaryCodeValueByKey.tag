	...
	<c:forEach items="${ex:dictList(key)}" var="item">
		<c:choose>
			<c:when test="${item.isdefault==1}">
				<option value="${item.description}-${item.value}" selected="selected">${item.description}-${item.value}</option>
			</c:when>
			<c:otherwise>
				<option value="${item.description}-${item.value}">${item.description}-${item.value}</option>
			</c:otherwise>
		</c:choose>
	</c:forEach>
</select>